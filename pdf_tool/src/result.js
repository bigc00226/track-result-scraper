...
(function (root, factory) {
    ...
    // 種目名の分解：「一般男子100m」→ 性別=男子 / 種別=一般 / 種目=100m（1.php と同じ）
    function splitKyogimei(title) {
        ...
        if (m) return { sex: map[m[2]], category: m[1], event: m[3] };
        return { sex: '', category: '', event: s };
    }
    ...
    function parse(pages) {
        ...
        if (!meet.name) warnings.push({ type: 'meet', message: '大会名が見つかりませんでした。' });
        return { meet: meet, events: events, rows: outRows, warnings: warnings, headers: HEADERS };
    }

    return {
        parse: parse, HEADERS: HEADERS,
